...
void sound_effect(int* , int*, int* );
void u8g_prepare(void);
void u8g2_prepare(void);
void Display(SNAKE_T, SNAKE_T, SNAKE_T);
...
void loop() {
  ...
  if( end_game == 0 ){
    ...
    y_data[2] = analogRead(A0);
    x_data[2] = analogRead(A1);                                                               
    
    for(int j = 0; j < PLAYERS; j++){
      if(Snake[j].looser == 0){
        Read_Joystick(x_data[j], y_data[j], &Snake[j]);          
        Move_Snake(&Snake[j], &musica, &cont, &Snake[3]);             
        there_is_fruit( &fruit_pos_x, &fruit_pos_y, &Snake[j], &musica, &cont);
      } 
    }          

    Display(&Snake[0], &Snake[1], &Snake[2]);
    ...
    delay(delaytime);
      ...
      do {
        ...
        u8g2.drawStr(9, 40, "PY 1: ");
        u8g2.drawStr(45, 40, "PY 2: ");
        u8g2.drawStr(80, 40, "PY 3: ");
...
void Display(SNAKE_T *Snake1, SNAKE_T *Snake2, SNAKE_T *Snake3){
  int k;
  u8g.firstPage();  
  do {
    ...
    for(k = 0; k < Snake3->history_size; k++){
      u8g.drawPixel(Snake3->history[k][0], Snake3->history[k][1]);
    }
